...
class Carver(threading.Thread):
    ...
    def run(self):
        try:
            ...
            # Linux block device size fallback
            if total_size == 0 and is_linux_block:
                try:
                    import subprocess
                    out = subprocess.check_output(["blockdev", "--getsize64", self.source_path], text=True)
                    total_size = int(out.strip())
                except:
                    try:
                        out = subprocess.check_output(["lsblk", "-b", "-n", "-o", "SIZE", self.source_path], text=True)
                        total_size = int(out.strip().split()[0])
                    except:
                        pass
            ...
            with open(self.source_path, "rb") as f_in:
                ...
                while not self._stop_event.is_set():
                    ...
                    try:
                        ...
                        # На Linux чтение/позиционирование за границей устройства вызывает OSError
                        if offset > 0:
                            print(f"[Info] Достигнут конец диска (смещение {offset}): {e}")
                            break
                        else:
                            raise e
                    ...
                    if earliest_idx == -1:
                        ...
                        offset += min(len(chunk), chunk_size)
                    ...
                    if file_size != -1 and file_size <= len(candidate_data):
                        ...
                        if file_hash in self.recovered_hashes:
                            # Already recovered this file content, skip saving it
                            offset = global_offset + file_size
                            continue
                        self.recovered_hashes.add(file_hash)
