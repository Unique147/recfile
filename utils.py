...
def get_drives():
    """
    Возвращает список физических и логических дисков.
    ...
    if os.name == 'nt':
        ...
        try:
            ...
            creation_flags = 0x08000000 # CREATE_NO_WINDOW constant value
        ...
        # Код для Linux / POSIX
        # Попробуем использовать lsblk
        try:
            output = subprocess.check_output(
                ["lsblk", "-o", "NAME,FSTYPE,LABEL,SIZE,MOUNTPOINT,TYPE,MODEL", "-b", "-J"],
                text=True
            )
            import json
            ...
            def process_device(dev):
                ...
                path = f"/dev/{name}"
                is_physical = (dev_type == 'disk')
                
                # Формируем описание
                if is_physical:
                    label_desc = f"{model} ({size_gb} GB)" if model else f"Физический диск {name} ({size_gb} GB)"
                else:
                    parts = []
                    if label:
                    ...
                    if mountpoint:
                        parts.append(f"Mount: {mountpoint}")
                    desc_str = ", ".join(parts)
                    if desc_str:
                        label_desc = f"Раздел {name} [{desc_str}] ({size_gb} GB)"
                    else:
                        label_desc = f"Раздел {name} ({size_gb} GB)"
            ...
            print(f"Ошибка при получении дисков через lsblk: {e}")
            # Резервный вариант: парсинг /proc/partitions
            try:
                        ...
                        if any(name.startswith(prefix) for prefix in ('sd', 'hd', 'vd', 'nvme', 'mmcblk')):
                            ...
                            if any(c.isdigit() for c in name):
                                if 'nvme' in name or 'mmcblk' in name:
                                    is_physical = not ('p' in name and name.split('p')[-1].isdigit())
                                else:
                                    is_physical = False
                            ...
                            path = f"/dev/{name}"
                            label = f"Диск {name} ({size_gb} GB)" if is_physical else f"Раздел {name} ({size_gb} GB)"
                            drives.append({
                                'path': path,
                                'label': label,
                                'is_physical': is_physical
                ...
                print(f"Резервный метод получения дисков не удался: {fe}")
